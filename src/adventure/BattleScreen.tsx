...
export const BattleScreen = ({ player, selectedBoss, finalStats, onBack, onGameOver }: BattleScreenProps) => {
    ...
    return (
        <div
            className="relative p-6 rounded-xl border border-slate-700 max-w-2xl mx-auto text-white overflow-hidden bg-cover bg-center"
            style={{ backgroundImage: `url(${currentBg})` }}
        >
            {/* Overlay สีดำจางๆ เพื่อให้อ่านข้อความและดู UI ง่ายขึ้น */}
            <div className="absolute inset-0 bg-slate-950/45 bg-gradient-to-b from-slate-950/20 via-slate-950/50 to-slate-950/80"></div>

            {/* เนื้อหา UI เดิมทั้งหมดต้องหุ้มด้วย relative z-10 เพื่อให้อยู่เหนือพื้นหลัง */}
